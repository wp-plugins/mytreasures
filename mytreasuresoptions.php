	...
	if(!current_user_can('level_10')) {
		...
		if(!is_writeable($path)) {
			...
			if(strlen($_POST[option01]) > 0) {
				...
				if(!preg_match("/^([0-9]+)$/",$_POST[option33]) || !$_POST[option33] || $_POST[option33] > 100) { $_POST[option33] = "25";  }
...
<br /><input name="option14" type="radio" value="no" <?php if($result01[option14] == '' || $result01[option14] == 'no') { echo "checked=\"checked\""; } ?>/> <?php echo __("Don't show big cover / image",$myTreasuresTextdomain); ?>
