	...
	if($checksystem) {
		...
		if($checkurlfopen) {
			...
			if($_POST[field01] && $_POST[treasuretype]) {
				...
				$returnarray = myTreasuresXML2Array2Return(myTreasuresXML2Array("http://xml.n4rf.net/ofdbgw/movie/".urlencode($_POST[ofdbid])));
				if($returnarray) {
					...
					if($check_doubletitle && $_POST[doit]) { $message .= "<br /><br /><b>".__("Important information",$myTreasuresTextdomain)."</b><br />".__("This title is already in database, continue adding?",$myTreasuresTextdomain)."<br /><input type=\"checkbox\" name=\"override\" value=\"1\"> ".__("Yes",$myTreasuresTextdomain)."<br />"; }
					for($i = 2; $i <= 20; $i++) {
				...
				if($_POST[search]) {
					...
					$message = "<form action=\"\" method=\"post\"><input type=\"text\" style=\"width: 70%;\" name=\"search\" /><div class=\"submit\"><input type=\"submit\" class=\"button-primary\" value=\"".__("Search...",$myTreasuresTextdomain)."\"></div></form>";
