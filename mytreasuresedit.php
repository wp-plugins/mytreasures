			...
			if($result01[id]) {
				...
				if($_POST[field01] && (!$result02[feature_tracklist] || ($result02[feature_tracklist] && $tracks))) { 
					...
					if(!$_POST) {
						...
						if(!$_POST[field01]) { $result02['field01'] = "<img src=\"../wp-content/plugins/mytreasures/images/missing.gif\" />".$result02['field01']; }
			...
			if($result01[id]) {
				...
				if($_POST[name] && $_POST[field01] && $_POST[doit]) {
					...
					for($i = 2; $i <= 20; $i++) {
						...
						if($i < 4) {
							if($_POST["listview_".$field] != '1') { $_POST["listview_".$field] = "0"; }
						}
