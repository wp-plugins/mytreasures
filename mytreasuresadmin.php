<?php
	...
	if($checksystem) {
		...
		if($_POST[changefilter]) { $_GET[showmedia] = $_POST[showmedia]; }
		if($_GET[showmedia]) { $extendquery = "WHERE `".$wpdb->prefix."mytreasures_type`.`id` = '".$_GET[showmedia]."'"; }
		if($_POST[showmedia]) { $extendlink = "&showmedia=".$_POST[showmedia]; } elseif($_GET[showmedia]) { $extendlink = "&showmedia=".$_GET[showmedia]; }
		if($order == 'title') { $italic1 = "font-style:italic;"; }
		if($order == 'type') { $italic2 = "font-style:italic;"; }
		if($order == 'id') { $italic3 = "font-style:italic;"; }
		if($myTreasuresMediaTypeArray) { foreach($myTreasuresMediaTypeArray AS $id => $name) { if($_POST[showmedia] == $id || $_GET[showmedia] == $id) { $selected = "selected=\"selected\""; } else { $selected = false; } $selectfilter .= "<option value=\"".$id."\" ".$selected.">".$name."</option>"; } }
		...
		if(mysql_num_rows($query01)) {
			...
			while($result01 = mysql_fetch_array($query01)) {
				...
				if($result01[rentto] && $myTreasures_options[option29] == 'yes') { $italic = "style=\"font-style: italic;\""; } else { $italic = false; }
				if($result01[rentto] && $myTreasures_options[option29] == 'yes') { $result01[field01] = "<b>".__("Rent to",$myTreasuresTextdomain).":</b> ".$result01[rentto]." - ".$result01[field01]; }
				if(!$result01[image]) { $result01[field01] .= " (<b>".__("No Image / Cover!",$myTreasuresTextdomain)."</b>)"; }
				$message .= "

<tr id=\"user-1\" ".$class.">
	<th scope=\"row\" class=\"check-column\"><input type=\"checkbox\" name=\"deletemedia[".$result01[id]."]\" id=\"user_1\" value=\"".$result01[field01]."\" /></th>
	<td class=\"username column-username\" ".$italic.">".$result01[field01]."<br /><div class=\"row-actions\"><span class=\"edit\"><a href=\"?page=mytreasures/mytreasuresedit.php&type=media&id=".$result01[id]."\">".__("Edit",$myTreasuresTextdomain)."</a> | <a href=\"?page=mytreasures/mytreasuresdelete.php&type=media&id=".$result01[id]."\">".__("Delete",$myTreasuresTextdomain)."</a> | <a href=\"?page=mytreasures/mytreasuresimages.php&id=".$result01[id]."\">".__("Administrate images",$myTreasuresTextdomain)."</a> | <a href=\"?page=mytreasures/mytreasureslinks.php&id=".$result01[id]."\">".__("Administrate links",$myTreasuresTextdomain)."</a></span></div></td>
	<td class=\"name column-name\">".$result01[name]."</td>
	<td class=\"posts column-posts num\">".$result01[id]."</td>
</tr>";
