	...
	if($checksystem) {
		...
		if($_POST[treasuretype]) {
			...
			foreach($myTreasuresMediaTypeArray AS $id => $name) {
				...
				$schoosemedia .= "<br /><input type=\"radio\" name=\"treasuretype\" value=\"".$id."\"> ".$name;

			}

			$message = "<form action=\"\" method=\"post\"><p>".__("Please choose which media type you want to add:",$myTreasuresTextdomain).$schoosemedia."</p><div class=\"submit\"><input type=\"submit\" class=\"button-primary\" value=\" ".__("Continue to details",$myTreasuresTextdomain)." \"></div></form>";
